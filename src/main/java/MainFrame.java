...
public class MainFrame {
	...
	public void createAndShowGUI() {
		...
		JMenu manageMenu = new JMenu("Manage Mods");
		manageMenu.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				new ManagerFrame().createAndShowGUI();
			}
		});
		...
		modList.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (index >= 0) {
					...
					if (selectedMods.contains(mod)) selectedMods.remove(mod);
					else selectedMods.add(mod);
					modList.repaint();
				}
			}
		});
	...
	void downloadSelected() {
		...
		for (Set<ModEntry> selections : pageSelections.values()) allSelected.addAll(selections);
